...
`./node_modules/.bin/jest` - это ссылка на исполняемый файл "Jest", а флаг `--init` говорит от том
что мы хотим проинициализировать конфигурацию "Jest".
...
Делать мы будем это через npm скрипты.
...
файл ".babelrc" о котором ранее сообщалось в ошибке от "Jest" при попытке запуска тестов.

А также добавить некоторые плагины. Давайте сделаем это.
